...
function Admin() {
  ...
  $request = db_query("
    ...
    WHERE ID_GROUP = 1
    OR FIND_IN_SET(1, additionalGroups)
  ...
  list ($context['current_versions']['mysql']['version']) = mysqli_fetch_row($request);
  mysqli_free_result($request);
...
function OptimizeTables() {
  ...
  if (!$result) {
    ...
    while ($table = mysqli_fetch_assoc($result))
      // TO-DO: Qué usar mysql_tablename
...
function Maintenance() {
  global $context, $txt, $db_prefix, $user_info, $db_character_set, $modSettings, $db_connection;
  ...
  } else if (isset($_GET['sa']) && $_GET['sa'] == 'destroy') {
    // Oh noes!
  ...
  $context['convert_utf8'] = (!isset($db_character_set) || $db_character_set !== 'utf8' || empty($modSettings['global_character_set']) || $modSettings['global_character_set'] !== 'UTF-8') && version_compare('4.1.2', preg_replace('~\-.+?$~', '', mysqli_get_server_info($db_connection))) <= 0;
...
function AdminBoardRecount() {
  global $txt, $db_prefix, $context, $modSettings, $time_start;
  ...
  list($max_topics) = mysqli_fetch_row($request);
  mysqli_free_result($request);
...
function VersionDetail() {
  ...
  $lang_dir = $boarddir . '/web/archivos/idiomas';
...
function updateSettingsFile($config_vars) {
  ...
  else if (strpos($settingsArray, "\r") !== false)
...
function ConvertUtf8() {
  global $context, $txt, $language, $db_prefix, $db_character_set;
  global $modSettings, $user_info, $sourcedir, $db_connection;
  ...
  if (!isset($_POST['proceed'])) {
    ...
    if (version_compare('4.1.2', preg_replace('~\-.+?$~', '', mysqli_get_server_info($db_connection))) > 0)
    ...
    $column_info = mysqli_fetch_assoc($request);
  ...
  while ($table_info = mysqli_fetch_assoc($queryTables)) {
    ...
    while ($column_info = mysqli_fetch_assoc($queryColumns)) {
    ...
    mysqli_free_result($queryColumns);
...
function ConvertEntities() {
  ...
  for (; $context['table'] < $context['num_tables']; $context['table']++) {
    ...
    while ($column_info = mysqli_fetch_assoc($request))
      if (strpos($column_info['Type'], 'text') !== false || strpos($column_info['Type'], 'char') !== false)
    ...
    list($max_value) = mysqli_fetch_row($request);
    mysqli_free_result($request);
