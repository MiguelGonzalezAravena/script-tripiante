...
function MainView()
{
	...
	if (isset($_REQUEST['cat']))
	{
		...
		WHERE
			seotitle = '$cat' LIMIT 1", __FILE__, __LINE__);
	...
	else
	{
		...
		$dbresult = db_query("
		SELECT 
			c.ID_CAT, c.title, c.imageurl, c.filename, c.roworder, c.description, p.view, c.seotitle
...
function GetArticleTotals($cat)
{
	...
	if ($modSettings['smfarticles_countsubcats'])
	{
		...
		$dbresult3 = db_query("
		SELECT 
			ID_CAT, total, title, seotitle
		FROM {$db_prefix}articles_cat 
		WHERE seotitle = '$cat' ORDER BY roworder ASC", __FILE__, __LINE__);
		while($row3 = mysql_fetch_assoc($dbresult3))
		{
			...
			if ($row3['total'] == -1)
			{
				...
				WHERE a.ID_CAT = ac.ID_CAT AND ac.seotitle = '$cat' AND ac.ID_CAT = " . $row3['ID_CAT'] . "", __FILE__, __LINE__);
				$row = mysql_fetch_assoc($dbresult);
				$total2 = $row['total'];
				mysql_free_result($dbresult);
				
				
				$dbresult = db_query("UPDATE {$db_prefix}articles_cat SET total = $total2 WHERE seotitle =  " . $row3['seotitle'] . " LIMIT 1", __FILE__, __LINE__);
			}
		}
...
function GetTotalByCATID($ID_CAT)
{
	global $db_prefix;
		...
		else
		{
			...
			// Update the count
			$dbresult = db_query("UPDATE {$db_prefix}articles_cat SET total = $total WHERE seotitle = '$cat' LIMIT 1", __FILE__, __LINE__);
...
function MyArticles()
{
	...
	// Show the articles in that category
		$dbresult = db_query("
		SELECT 
			a.ID_ARTICLE, a.title, a.date, a.approved, a.totalratings, m.realName, a.ID_MEMBER, a.description, a.views  
...
function Search()
{
	...
	$dbresult = db_query("
	SELECT 
		c.ID_CAT, c.title, p.view, c.ID_PARENT, c.seotitle 
	FROM {$db_prefix}articles_cat as c 
	LEFT JOIN {$db_prefix}articles_catperm AS p ON (p.ID_GROUP = $groupid AND c.ID_CAT = p.ID_CAT)  
	ORDER BY c.title ASC", __FILE__, __LINE__);
	$context['articles_cat'] = array();
...
function Search2()
{
    ...
    $dbresult = db_query("
    SELECT 
    	p.ID_ARTICLE, p.title, p.date, p.totalratings, m.realName, p.ID_MEMBER, p.description, p.views   
    FROM {$db_prefix}articles as p 
...
function ViewArticle()
{
	...
	$article = (int) htmlentities(addslashes($_REQUEST['article']));
