...
function Display() {
  ...
  if (isset($_REQUEST['tags']) && !isset($_REQUEST['num_replies'])) {
    ...
    $tags = explode(',', htmlentities($_REQUEST['tags'], ENT_QUOTES, 'UTF-8'));
    ...
    if ($totaltags < $modSettings['smftags_set_maxtags']) {
      ...
      foreach($tags as $tag) {
        ...
        if (db_affected_rows() == 0) {
          db_query("
            INSERT INTO {$db_prefix}tags (tag)
            VALUES ('$tag')", __FILE__, __LINE__);
  ...
  $context['page_title'] = censorText($topicinfo['subject']);
  ...
  if (!empty($messages)) {
    ...
    $request = db_query("
      SELECT t.ID_TAG
      FROM ({$db_prefix}tags_log AS tl INNER JOIN {$db_prefix}tags AS t ON tl.ID_TAG = t.ID_TAG)
      INNER JOIN {$db_prefix}messages AS m ON m.ID_TOPIC = tl.ID_TOPIC
    ...
    $rs = db_query("
      ...
      WHERE o.ID_TOPIC = $topic
      AND o.TYPE = 'posts'", __FILE__, __LINE__);
