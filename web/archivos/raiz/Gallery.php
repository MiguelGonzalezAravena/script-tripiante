...
function ViewPicture() {
  global $context, $db_prefix, $modSettings, $user_info, $scripturl, $txt;
  ...
  if (!empty($modSettings['gallery_who_viewing'])) {
    ...
    if(function_exists('parse_bbc')) {
      ...
      $context['view_num_guests'] = mysqli_num_rows($request) - count($context['view_members']);
...
function AddPicture2() {
  global $ID_MEMBER, $txt, $db_prefix, $modSettings, $context, $boardurl;
  ...
  if ($context['Turista']) {
    ...
    echo '<div align="center"><div class="box_errors"><div class="box_title" style="width: 388px"><div class="box_txt box_error" align="left">&iexcl;Atenci&oacute;n!</div><div class="box_rss"><img alt="" src="' . $settings['images_url'] . '/blank.gif" style="width: 14px; height: 12px;" border="0" /></div></div><div class="windowbg" style="width:380px;padding:4px;"><br />Debes agregar un t&iacute;tulo.-<br /><br /><input class="login" style="font-size: 11px;" type="button" title="Volver atr&aacute;s" value="Volver atr&aacute;s" onclick="history.back()" /> <input class="login" style="font-size: 11px;" type="submit" title="Ir a la p&aacute;gina principal" value="Ir a la p&aacute;gina principal" onclick="location.href=\'/\'" /><br /><br /></div></div><br /></div><div style="clear:both"></div>';
  } else if (empty($filename)) {
    echo '<div align="center"><div class="box_errors"><div class="box_title" style="width: 388px"><div class="box_txt box_error" align="left">&iexcl;Atenci&oacute;n!</div><div class="box_rss"><img alt="" src="' . $settings['images_url'] . '/blank.gif" style="width: 14px; height: 12px;" border="0" /></div></div><div class="windowbg" style="width:380px;padding:4px;"><br />Debes agregar una imagen.-<br /><br /><input class="login" style="font-size: 11px;" type="button" title="Volver atr&aacute;s" value="Volver atr&aacute;s" onclick="history.back()" /> <input class="login" style="font-size: 11px;" type="submit" title="Ir a la p&aacute;gina principal" value="Ir a la p&aacute;gina principal" onclick="location.href=\'/\'" /><br /><br /></div></div><br /></div><div style="clear:both"></div>';
  } else {
    db_query("
      INSERT INTO {$db_prefix}gallery_pic (filename, title, ID_MEMBER, date)
      VALUES ('$filename', '$title', '$ID_MEMBER', '$t')", __FILE__, __LINE__);

    if (isset($modSettings['shopVersion']))
      db_query("
        UPDATE {$db_prefix}members
        SET
          money = money + " . $modSettings['gallery_shop_picadd'] . ",
          moneyBank = moneyBank + " . $modSettings['gallery_shop_picadd'] . "
...
function EditPicture()
{
  ...
  $dbresult = db_query("
    SELECT p.ID_PICTURE, p.thumbfilename, p.allowcomments, p.commenttotal, p.filesize, p.filename, p.approved, p.views, p.title, p.ID_MEMBER, m.memberName, m.realName, p.date, p.description
    FROM {$db_prefix}gallery_pic as p
...
function EditPicture2() {
  ...
  $dbresult = db_query("
    SELECT ID_MEMBER, title, filename
  ...
  if (AllowedTo('smfgallery_manage') || (AllowedTo('smfgallery_edit') && $ID_MEMBER == $memID)) {
    ...
    db_query("
      UPDATE {$db_prefix}gallery_pic
      SET title = '$title', filename = '$filename'
      WHERE ID_PICTURE = $id
      LIMIT 1", __FILE__, __LINE__);
    ...
    if (!empty($modSettings['modlog_enabled']) && allowedTo('modify_any')) {
      db_query("
        INSERT INTO {$db_prefix}mod_history (ID_MODERATOR, ID_MEMBER, ID_TOPIC, TYPE, ACTION, subject, reason)
        VALUES ('" . $ID_MODERATOR . "', '" . $ID_MEMBER . "', '" . $ID_TOPIC . "', '" . $TYPE . "', '" . $ACTION . "', '" . $subject . "', '" . $reason . "')", __FILE__, __LINE__);
    }
...
function DeletePicture() {
  ...
  $dbresult = db_query("
    SELECT p.ID_PICTURE, p.thumbfilename, p.width, p.height, p.allowcomments, p.ID_CAT, p.keywords, p.commenttotal, p.filesize, p.filename, p.approved, p.views, p.title, p.ID_MEMBER, m.memberName, m.realName, p.date, p.description
    FROM {$db_prefix}gallery_pic as p
    LEFT JOIN {$db_prefix}members AS m ON (m.ID_MEMBER = p.ID_MEMBER)
    WHERE ID_PICTURE = $id
    LIMIT 1", __FILE__, __LINE__);
...
function DeletePicture2() {
  ...
  // Check if the user owns the picture or is admin
  $dbresult = db_query("
    SELECT p.ID_PICTURE, p.filename, p.ID_MEMBER, p.title
    FROM {$db_prefix}gallery_pic as p
    WHERE ID_PICTURE = $id
    LIMIT 1", __FILE__, __LINE__);
  ...
  if(AllowedTo('smfgallery_manage') || (AllowedTo('smfgallery_delete') && $ID_MEMBER == $memID)) {
    ...
    list($starter, $subject) = mysqli_fetch_row($requesti);
    mysqli_free_result($requesti);
...
function dpuntos() {
  global $context, $txt, $db_prefix, $boardurl, $boardurl;
  ...
  $request = db_query("
    ...
    WHERE m.ID_MEMBER = " . $context['user']['id'], __FILE__, __LINE__);
  ...
  // TO-DO: ¿Existe esta tabla gallery_cat?
  /*
  db_query("
    INSERT INTO {$db_prefix}gallery_cat(id_img, id_user)
    VALUES ('$id', '$userincr')", __FILE__, __LINE__);
  */
...
function AddComment2() {
  global $db_prefix, $ID_MEMBER, $txt, $modSettings, $boardurl;
...
function DeleteComment() {
  ...
  if(!empty($_POST['campos'])) {
    ...
    db_query("
      UPDATE {$db_prefix}gallery_pic
      SET commenttotal = commenttotal - $total
      WHERE ID_PICTURE = $idimg
      LIMIT 1", __FILE__, __LINE__);
  }
...
function DeleteReport() {
  ...
  db_query("
    DELETE FROM {$db_prefix}gallery_report
    WHERE ID = $id
    LIMIT 1", __FILE__, __LINE__);
