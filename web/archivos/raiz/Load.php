...
function loadUserSettings()
{
	...
	$user_info += array(
		'username' => $username,
		'nombre' => isset($user_settings['name']) ? $user_settings['name'] : '',
		'estado_icon' => isset($user_settings['estado_icon']) ? $user_settings['estado_icon'] : '',
...
function loadBoard()
{
	...
	global $board_info, $board, $topic, $ID_MEMBER, $user_info, $sourcedir;
	...
	if (empty($temp))
	{
		$request = db_query("
			SELECT
				...
				b.ID_THEME, b.override_theme, mem.estado_icon, b.permission_mode, b.countPosts
...
function loadMemberData($users, $is_name = false, $set = 'normal')
{
	...
	if ($set == 'normal')
	{
		$select_columns = "
			...
			mem.realName, mem.emailAddress, mem.hideEmail, mem.dateRegistered, mem.websiteTitle, mem.quienve, mem.sidebar, mem.recibir, mem.estudios, mem.profesion, mem.empresa, mem.ingresos, mem.intereses_profesionales, mem.habilidades_profesionales, mem.websiteUrl,
			mem.birthdate, mem.memberIP, mem.memberIP2, mem.ICQ, mem.AIM, mem.YIM, mem.MSN, mem.posts, mem.topics, mem.me_gustaria, mem.estado, mem.hijos, mem.mis_intereses, mem.hobbies, mem.series_tv_favoritas, mem.musica_favorita, mem.deportes_y_equipos_favoritos, mem.libros_favoritos, mem.peliculas_favoritas, mem.comida_favorita, mem.mis_heroes_son, mem.lastLogin,
	...
	elseif ($set == 'profile')
	{
		$select_columns = "
			IFNULL(lo.logTime, 0) AS isOnline, mem.signature, mem.personalText, mem.location, mem.gender, mem.avatar, mem.ID_MEMBER, mem.name, mem.memberName, mem.estado_icon,
			mem.realName, mem.emailAddress, mem.hideEmail, mem.dateRegistered, mem.websiteTitle, mem.quienve, mem.sidebar, mem.me_gustaria, mem.estado, mem.hijos, mem.recibir, mem.mis_intereses, mem.hobbies, mem.series_tv_favoritas, mem.musica_favorita, mem.deportes_y_equipos_favoritos, mem.libros_favoritos, mem.peliculas_favoritas, mem.comida_favorita, mem.mis_heroes_son, mem.websiteUrl,
			mem.birthdate, mem.ICQ, mem.AIM, mem.YIM, mem.MSN, mem.posts, mem.topics, mem.lastLogin, mem.estudios, mem.profesion, mem.empresa, mem.ingresos, mem.intereses_profesionales, mem.habilidades_profesionales, 
			mem.memberIP, mem.memberIP2, mem.lngfile, mem.ID_GROUP, mem.ID_THEME, mem.altura, mem.peso, mem.pelo_color, mem.ojos_color, mem.fisico, mem.dieta, mem.fumo, mem.tomo_alcohol, mem.buddy_list, mem.pm_ignore_list,
	...
	elseif ($set == 'minimal')
	{
		$select_columns = '
			...
			mem.posts, mem.topics, mem.lastLogin, mem.memberIP, mem.memberIP2, mem.quienve, mem.sidebar, mem.recibir, mem.me_gustaria, mem.estado, mem.hijos, mem.altura, mem.peso, mem.pelo_color, mem.ojos_color, mem.fisico, mem.dieta, mem.fumo, mem.tomo_alcohol, mem.lngfile,  mem.estado_icon, mem.ID_GROUP';
		$select_tables = '';
	}
	else
...
function loadMemberContext($user)
{
	...
	$memberContext[$user] = array(
		...
		'nombre' => &$profile['name'],
		'estado_icon' => &$profile['estado_icon'],
		'sidebar' => &$profile['sidebar'],
		'quienve' => &$profile['quienve'],
		'recibir' => &$profile['recibir'],
		...
		'gender' => array(
			'name' => $gendertxt,
			'image' => !empty($profile['gender']) ? '<img src="' . $settings['images_url'] . '/' . ($profile['gender'] == 1 ? 'Male' : 'Female') . '.gif" alt="" title="' . $gendertxt . '" border="0" />' : ''
		),
		'website' => array(
	...
	array('name' => '', 'href' => '', 'link' => '', 'link_text' => ''),
		...
		'group_stars' => str_repeat('<img src="' . str_replace('$language', $context['user']['language'], isset($profile['stars'][1]) ? $settings['images_url'] . '/' . $profile['stars'][1] : '') . '" alt="" title="' . $profile['member_group'] . '" border="0" />', empty($profile['stars'][0]) || empty($profile['stars'][1]) ? 0 : $profile['stars'][0]),
...
function loadLanguage($template_name, $lang = '', $fatal = true)
{
	...
	foreach ($attempts as $k => $file)
		if (file_exists($boarddir . '/web/archivos/idiomas/' . $file[1] . '.' . $file[2] . '.php'))
		{
			$language_dir = $boarddir . '/web/archivos/idiomas';
			$lang = $file[2];
			// Hmmm... do we really still need this?
			$language_url = $file[3];
			template_include($boarddir . '/web/archivos/idiomas/' . $file[1] . '.' . $file[2] . '.php');
