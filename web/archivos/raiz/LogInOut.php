...
function Login2() {
  ...
  if (isset($_GET['sa']) && $_GET['sa'] == 'salt' && !$user_info['is_guest']) {
    ...
    else if (isset($_SESSION['login_' . $cookiename]))
  ...
  else if (isset($_GET['sa']) && $_GET['sa'] == 'check') {
  ...
  else if (!empty($_POST['cookielength']) && ($_POST['cookielength'] >= 1 || $_POST['cookielength'] <= 525600))
  ...
  if (isset($_REQUEST['hash_passwrd']) && strlen($_REQUEST['hash_passwrd']) == 40) {
    ...
    // Challenge passed.
    else if ($_REQUEST['hash_passwrd'] == sha1($user_settings['passwd'] . $sc))
  ...
  else if ($user_settings['passwordSalt'] == '') {
    $user_settings['passwordSalt'] = substr(md5(mt_rand()), 0, 4);
