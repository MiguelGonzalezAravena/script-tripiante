...
function PackageServers() {
  global $txt, $context, $boarddir, $modSettings, $db_prefix;
  ...
  {
    @chmod($boarddir . '/web/archivos/paquetes', 0777);
    @chmod($boarddir . '/web/archivos/paquetes/installed.list', 0777);
  }
...
function PackageDownload()
{
  ...
  if (isset($_REQUEST['conflict']) || (isset($_REQUEST['auto']) && file_exists($boarddir . '/web/archivos/paquetes/' . $package_name)))
  {
    ...
    $i = 1;
    while (file_exists($boarddir . '/web/archivos/paquetes/' . $package_name . $i . $ext))
      $i++;
  ...
  packageRequireFTP($scripturl . '?action=packageget;sa=download' . (isset($_GET['server']) ? ';server=' . $_GET['server'] : '') . (isset($_REQUEST['auto']) ? ';auto' : '') . ';package=' . $_REQUEST['package'] . (isset($_REQUEST['conflict']) ? ';conflict' : '') . ';sesc=' . $context['session_id'], array($boarddir . '/web/archivos/paquetes/' . $package_name));
  package_put_contents($boarddir . '/web/archivos/paquetes/' . $package_name, fetch_web_data($url . $_REQUEST['package']));
...
function PackageUpload()
{
  ...
  $destination = $boarddir . '/web/archivos/paquetes/' . $packageName;
