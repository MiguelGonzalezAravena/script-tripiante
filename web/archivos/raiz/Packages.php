...
function PackageInstallTest()
{
  ...
  if (file_exists($boarddir . '/web/archivos/paquetes/temp'))
    deltree($boarddir . '/web/archivos/paquetes/temp', false);

  if (!mktree($boarddir . '/web/archivos/paquetes/temp', 0755))
  {
    deltree($boarddir . '/web/archivos/paquetes/temp', false);
    if (!mktree($boarddir . '/web/archivos/paquetes/temp', 0777))
    {
      deltree($boarddir . '/web/archivos/paquetes/temp', false);
      packageRequireFTP($scripturl . '?action=packages;sa=' . $_REQUEST['sa'] . ';package=' . $_REQUEST['package'], array($boarddir . '/web/archivos/paquetes/temp/delme.tmp'));
  ...
  if (!file_exists($boarddir . '/web/archivos/paquetes/' . $context['filename']))
  {
    deltree($boarddir . '/web/archivos/paquetes/temp');
  ...
  {
    $context['extracted_files'] = read_tgz_file($boarddir . '/web/archivos/paquetes/' . $context['filename'], $boarddir . '/web/archivos/paquetes/temp');

    if ($context['extracted_files'] && !file_exists($boarddir . '/web/archivos/paquetes/temp/package-info.xml'))
  ...
  foreach ($actions as $action)
  {
    ...
    elseif ($action['type'] == 'modification')
    {
      ...
      if ($action['boardmod'])
        $mod_actions = parseBoardMod(@file_get_contents($boarddir . '/web/archivos/paquetes/temp/' . $context['base_path'] . $action['filename']), true, $action['reverse']);
      else
        $mod_actions = parseModification(@file_get_contents($boarddir . '/web/archivos/paquetes/temp/' . $context['base_path'] . $action['filename']), true, $action['reverse']);
...
function PackageInstall()
{
  ...
  if (file_exists($boarddir . '/web/archivos/paquetes/temp'))
    ...
    mktree($boarddir . '/web/archivos/paquetes/temp', 0777);
  ...
  if (is_file($boarddir . '/web/archivos/paquetes/' . $context['filename']))
  {
    ...
    if (!file_exists($boarddir . '/web/archivos/paquetes/temp/package-info.xml'))
      foreach ($context['extracted_files'] as $file)
        if (basename($file['filename']) == 'package-info.xml')
        {
          $context['base_path'] = dirname($file['filename']) . '/';
  ...
  if (!empty($install_log))
  {
    foreach ($install_log as $action)
    {
      ...
      {
        if ($action['boardmod'])
          parseBoardMod(file_get_contents($boarddir . '/web/archivos/paquetes/temp/' . $context['base_path'] . $action['filename']), false, $action['reverse']);
        else
...
function FlushInstall()
{
  ...
  package_put_contents($boarddir . '/web/archivos/paquetes/installed.list', '');
...
function PackageRemove()
{
  ...
  if (file_exists($boarddir . '/web/archivos/paquetes/' . $_GET['package']) && (substr($_GET['package'], -4) == '.zip' || substr($_GET['package'], -4) == '.tgz' || substr($_GET['package'], -7) == '.tar.gz' || is_dir($boarddir . '/web/archivos/paquetes/' . $_GET['package'])) && $_GET['package'] != 'backups' && substr($_GET['package'], 0, 1) != '.')
  {
    packageRequireFTP($scripturl . '?action=packages;sa=remove;package=' . $_GET['package'], array($boarddir . '/web/archivos/paquetes/' . $_GET['package']));

    if (is_dir($boarddir . '/web/archivos/paquetes/' . $_GET['package']))
      deltree($boarddir . '/web/archivos/paquetes/' . $_GET['package']);
    else
    {
      @chmod($boarddir . '/web/archivos/paquetes/' . $_GET['package'], 0777);
      unlink($boarddir . '/web/archivos/paquetes/' . $_GET['package']);
    }
  }
...
function PackageBrowse()
{
  ...
  if ($dir = @opendir($boarddir . '/web/archivos/paquetes'))
  {
    $dirs = array();
    while ($package = readdir($dir))
    {
      if ($package == '.' || $package == '..' || $package == 'temp' || (!(is_dir($boarddir . '/web/archivos/paquetes/' . $package) && file_exists($boarddir . '/web/archivos/paquetes/' . $package . '/package-info.xml')) && substr($package, -7) != '.tar.gz' && substr($package, -4) != '.tgz' && substr($package, -4) != '.zip'))
        continue;

      // Skip directories or files that are named the same.
      if (is_dir($boarddir . '/web/archivos/paquetes/' . $package))
      {
