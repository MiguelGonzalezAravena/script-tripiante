...
function MessageFolder()
{
  ...
  $context['page_index'] = constructPageIndex($boardurl . '/mensajes/' . $context['folder'] . (isset($_REQUEST['l']) ? ';l=' . (int) $_REQUEST['l'] : '') . ';sort=' . $context['sort_by'] . (isset($_GET['desc']) ? ';desc' : ''), $_GET['start'], $max_messages, $modSettings['defaultMaxMessages']);
...
if (isset($context['sl-singlepm']) && empty($pms))
    ...
    $messages_request = db_query("
      ...
      GROUP BY pm.ID_PM" : '') . "
      ORDER BY $_GET[sort] " . ($descending ? ' DESC' : ' DESC') . "
...
{
  global $txt, $scripturl, $modSettings, $context, $messages_request, $memberContext, $recipients, $boardurl;
...
function ReportMessage()
{
  ...
  else
  {
    ...
    if (mysqli_num_rows($request) == 0)
      fatal_lang_error(1, false);
    list ($subject, $body, $time, $memberFromID, $memberFromName) = mysqli_fetch_row($request);
    mysqli_free_result($request);
