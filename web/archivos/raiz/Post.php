  ...
  if (isset($_REQUEST['message']) || !empty($context['post_error'])) {
    ...
    if (empty($context['post_error'])) {
      ...
      if ($user_info['is_guest'])
      {
        ...
        else if (strlen($_REQUEST['guestname']) > 25)
    ...
    $form_subject = strtr(stripslashes($_REQUEST['subject']), array("\r" => '', "\n" => '', "\t" => ''));
    $form_message = stripslashes($_REQUEST['message'], ENT_QUOTES);

    if (strlen($form_subject) > 100) {
      $form_subject = substr($form_subject, 0, 100);
    }

    if (trim($form_subject) === '') {
      $context['post_error']['no_subject'] = true;
    }
    ...
    $form_subject = trim(htmlentities($row['subject'], ENT_QUOTES, 'ISO-8859-1'));
    // TO-DO: Comprobar si esto es lo que hace fallar el traer información del cuerpo
    // No funciona
    $form_message = trim(htmlentities(un_preparsecode($row['body']), ENT_QUOTES, 'ISO-8859-1'));
    // Funciona
    ...
    if (!empty($topic) && !empty($_REQUEST['quote'])) {
      ...
      if (trim($context['response_prefix']) != '' && strpos($form_subject, trim($context['response_prefix'])) !== 0) {
      ...
      if (trim($context['response_prefix']) != '' && $form_subject != '' && strpos($form_subject, trim($context['response_prefix'])) !== 0) {
        $form_subject = $context['response_prefix'] . $form_subject;
      }
...
function Post2() {
  ...
  if (!isset($_POST['message']) || trim($_POST['message']) === '') {
  ...
  } else if (!empty($modSettings['max_messageLength']) && strlen($_POST['message']) > $modSettings['max_messageLength']) {
    ...
    if (trim(strip_tags(parse_bbc($_POST['message'], false), '<img>')) === '') {
      $post_errors[] = 'no_message';
    }
  }
  ...
  if (!empty($_REQUEST['topic'])) {
    $history = db_query("
      SELECT t.ID_TOPIC, m.subject, t.ID_MEMBER_STARTED, m.ID_TOPIC
      FROM ({$db_prefix}topics AS t, {$db_prefix}messages AS m)
      WHERE t.ID_TOPIC = $topic
...
function getTopic() {
  global $topic, $db_prefix, $modSettings, $context, $sourcedir;
  ...
  if (isset($_REQUEST['xml'])) {
    ...
    $limit = empty($modSettings['topicSummaryPosts']) ? '' : 'LIMIT ' . (int) $modSettings['topicSummaryPosts'];
  }
