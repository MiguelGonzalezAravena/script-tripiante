...
function ModifyProfile2()
{
  ...
  global $cookiename, $context, $boardurl;
  global $sourcedir, $scripturl, $db_prefix, $boardurl;
  global $ID_MEMBER, $user_info, $boardurl;
  global $context, $newpassemail, $user_profile, $validationCode, $boardurl;
...
function summary($memID)
{
  ...
  if (isset ($modSettings['enable_buddylist']) && $modSettings['enable_buddylist'] == '1') {
    $buddies = array();
    $request = db_query ("SELECT * FROM {$db_prefix}buddies AS b
...
/* En comun */
  ...
  if (isset ($modSettings['enable_buddylist']) && $modSettings['enable_buddylist'] == '1') {
    $buddies = array();
    $request = db_query ("SELECT * FROM ({$db_prefix}members AS mem, {$db_prefix}buddies AS b, {$db_prefix}members AS mem2, {$db_prefix}buddies AS b2)
WHERE b.ID_MEMBER = " . $context['member']['id'] . "
...
function paso1($memID)
{
  ...
  global $user_info, $txt, $ID_MEMBER, $modSettings, $slogan;
...
function apariencia($memID)
{
  ...
  if (isset ($modSettings['enable_buddylist']) && $modSettings['enable_buddylist'] == '1') {
    $buddies = array();
    $request = db_query ("SELECT * FROM ({$db_prefix}buddies AS b, {$db_prefix}members AS mem)
        WHERE b.ID_MEMBER = " . $context['member']['id'] . "
        ORDER BY RAND()
        LIMIT 6", __FILE__, __LINE__);
    while ($row = mysql_fetch_assoc ($request))
...
{
  global $user_info, $txt, $ID_MEMBER, $modSettings, $slogan, $context, $settings;
...
function misnotas()
{
  global $db_prefix, $context, $txt, $notes, $ID_MEMBER, $modSettings, $smcFunc;
...
<html xmlns="http://www.w3.org/1999/xhtml"', $context['right_to_left'] ? ' dir="rtl"' : '', '>
  ...
  <body>
    ...
    if($_POST['sa'] == 'add' && !isset($error['no_body']) && !isset($error['change_body']))
    {
      ...
      else
      {
        ...
        db_query("
          INSERT INTO {$db_prefix}member_notes 
            (id_member, subject, body, posterTime) 
          VALUES ($id_member, '$_POST[titulo]', '$_POST[contenido]', '$time')", __FILE__, __LINE__);
        $id = db_insert_id();
        Header('Location: /mis-notas/');
      }
    }
    elseif($_POST['sa'] == 'edit' && !empty($id) && !isset($error['no_body']))
    {
      if (isset($smcFunc))
      {
        $smcFunc['db_query']('', '
  ...
  elseif((isset($_GET['sa']) && $_GET['sa'] == 'delete') || empty($_COOKIE['Notes']) || !empty($context['notes_error']))
...
{
  global $modSettings, $context, $db_prefix, $user_profile, $scripturl, $ID_MEMBER;
  global $context, $memberContext, $txt, $modSettings, $user_info, $user_profile, $sourcedir, $db_prefix, $scripturl;
  ...
  if (isset ($modSettings['enable_buddylist']) && $modSettings['enable_buddylist'] == '1') {
    ...
    $request = db_query ("SELECT * FROM ({$db_prefix}buddies AS b, {$db_prefix}members AS mem)
        WHERE b.ID_MEMBER = " . $context['member']['id'] . "
        ORDER BY b.position ASC, b.time_updated DESC", __FILE__, __LINE__);
    while ($row = mysql_fetch_assoc ($request))
    $buddies[] = $row['BUDDY_ID'];
    loadMemberData($buddies);
    foreach ($buddies as $buddy) {
