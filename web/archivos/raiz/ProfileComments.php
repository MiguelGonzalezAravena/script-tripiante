...
{
	global $memberContext, $settings, $modSettings, $txt, $context, $scripturl;

$RegistrosAMostrar		=	$modSettings['profile_comments_limit'];
if(isset($_GET['pag'])){
$RegistrosAEmpezar	=	((int) $_GET['pag']-1)*$RegistrosAMostrar;
$PagAct	=	(int) $_GET['pag'];
} else {
$RegistrosAEmpezar	=	0;
$PagAct				=	1;
...
$request	=	db_query("SELECT m.ID_MEMBER, m.realName, m.memberName, p.ID_MEMBER, p.ID_COMMENT, p.COMMENT_MEMBER_ID, p.date
...
$request	=	db_query("SELECT m.ID_MEMBER, m.realName, m.memberName, m.avatar, p.ID_MEMBER, p.ID_COMMENT, p.subject, p.COMMENT_MEMBER_ID, p.date, p.comment
FROM ({$db_prefix}members AS m, {$db_prefix}profile_comments AS p)
WHERE p.ID_MEMBER = m.ID_MEMBER
AND p.COMMENT_MEMBER_ID = " . $context['member']['id'] . "
...
$NroRegistros=mysql_num_rows(db_query("SELECT * FROM ({$db_prefix}members AS m, {$db_prefix}profile_comments AS p) WHERE p.ID_MEMBER = m.ID_MEMBER AND p.COMMENT_MEMBER_ID = " . $context['member']['id'] . " ", __FILE__, __LINE__));
}
 $PagAnt=$PagAct-1;
 $PagSig=$PagAct+1;
