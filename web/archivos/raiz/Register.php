...
function Captcha()
{
	...
	elseif(!is_numeric($captchaCode))
	{
	echo 'Tiene que ser un número.-';
	}
	elseif(strlen($captchaCode) > 4)
	{
	echo 'Solo están permitidos hasta 4 números.-';
	}
	else
	{
    // Genero el codigo y lo guardo en la sesión para consultarlo luego.
    $_SESSION['code'] = $captchaCode;
    
    // Genero la imagen
	$font = imageloadfont("../captcha/verdana.gdf");
	$img = imagecreatefrompng("../captcha/background.png");
    
    // Colores
    $stringColor = imagecolorallocate($img, 255, 199, 3);
     
	// Escribo el código
    imageString($img, $font, 25, -6, $captchaCode, $stringColor);
...
function Register2()
{
	...
	$possible_strings = array(
		...
		'pm_ignore_list',
		'smileySet',
		'signature', 'usertitle', 'name', 'personalText', 'avatar',
		'lngfile',
