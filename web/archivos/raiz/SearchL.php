...
function PlushSearch2() {
  ...
  {
    $userString = strtr(addslashes(htmlspecialchars(stripslashes($search_params['userspec']), ENT_QUOTES)), array('&quot;' => '"'));
    ...
    if (mysqli_num_rows($request) > $maxMembersToSearch)
      $userQuery = '';
    else if (mysqli_num_rows($request) == 0)
  ...
  if (!empty($context['topics']))
  {
    ...
    $messages_request = db_query("
      SELECT
        ...
        m.icon, m.posterIP, m.body, m.smileysEnabled, m.modifiedTime, m.modifiedName, t.points, 
