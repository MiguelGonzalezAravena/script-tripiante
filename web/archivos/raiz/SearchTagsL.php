...
function PlushSearch1()
{
	...
	if (!empty($context['search_params']['topic']))
	{
		...
		$context['search_topic'] = array(
			...
			'href' => $scripturl . '?topic=' . $context['search_params']['topic'],
		);

		$request = db_query("
			SELECT ms.subject, ms.body, tl.ID_TAG, tg.ID_TAG, tl.ID_TOPIC, tg.tag, t.ID_TOPIC
			FROM ({$db_prefix}topics AS t, {$db_prefix}boards AS b, {$db_prefix}messages AS ms, {$db_prefix}tags AS tg, {$db_prefix}tags_log AS tl)
			WHERE b.ID_BOARD = t.ID_BOARD
				AND t.ID_TOPIC = " . $context['search_params']['topic'] . "
				AND tl.ID_TAG = tg.ID_TAG
				AND tl.ID_TOPIC = t.ID_TOPIC
...
function PlushSearch2()
{
	...
	if (!empty($search_params['topic']))
	{
		$request = db_query("
			SELECT b.ID_BOARD
			FROM ({$db_prefix}topics AS t, {$db_prefix}boards AS b, {$db_prefix}tags as tg, {$db_prefix}tags_log AS tl)
			WHERE b.ID_BOARD = t.ID_BOARD
				AND t.ID_TOPIC = " . $search_params['topic'] . "
				AND tg.ID_TAG = tl.ID_TAG
				AND tl.ID_TOPIC = t.ID_TOPIC
				AND $user_info[query_see_board]
			LIMIT 1", __FILE__, __LINE__);
		...
		else
		{
			$main_query = array(
				...
				'from' => array(
					...
					"{$db_prefix}tags AS tg",
					"{$db_prefix}tags_log AS tgl",
				),
				'left_join' => array(),
				'where' => array(
					't.ID_TOPIC = m.ID_TOPIC',
					'tgl.ID_TOPIC = t.ID_TOPIC',
					'tgl.ID_TOPIC = m.ID_TOPIC',
				),
				'group_by' => array(),
			);
			...
			if (empty($search_params['topic']))
			{
				...
				$main_query['select']['points'] = 't.points';
	...
	if (!empty($context['topics']))
	{
		...
		$messages_request = db_query("
			SELECT
				m.ID_MSG, m.subject, m.posterName, m.posterEmail, m.posterTime, t.points, m.ID_MEMBER,
				m.icon, m.posterIP, m.body, m.smileysEnabled, m.modifiedTime, m.modifiedName,
				first_m.ID_MSG AS first_msg, first_mem.memberName AS memberName, first_m.subject AS first_subject, first_m.icon AS firstIcon, first_m.posterTime AS first_posterTime,
				...
				t.ID_TOPIC, t.isSticky, t.locked, t.ID_POLL, t.numReplies, t.numViews, tg.ID_TAG, tg.tag, tl.ID_TAG, tl.ID_TOPIC,
				b.ID_BOARD, b.name AS bName, b.description, c.ID_CAT, c.name AS cName
			FROM ({$db_prefix}messages AS m, {$db_prefix}topics AS t, {$db_prefix}boards AS b, {$db_prefix}categories AS c, {$db_prefix}messages AS first_m, {$db_prefix}messages AS last_m, {$db_prefix}tags AS tg, {$db_prefix}tags_log AS tl)
			...
			WHERE m.ID_MSG IN (" . implode(', ', array_keys($context['topics'])) . ")
				...
				AND tg.ID_TAG = tl.ID_TAG
				AND tl.ID_TOPIC = t.ID_TOPIC
				AND tl.ID_TOPIC = m.ID_TOPIC
			GROUP BY t.ID_TOPIC DESC
...
function prepareSearchContext($reset = false)
{
	...
	$output = array_merge($context['topics'][$message['ID_MSG']], array(
		'first_post' => array(
			...
			'puntos' => $message['points'],
			'fecha' => timeformat($message['first_posterTime']),
			...
			'member' => array(
				...
				'name' => $message['first_member_name'],
				'nombre' => $message['memberName'],
				'href' => !empty($message['first_member_id']) ? $boardurl . '/perfil/' . $message['memberName'] : '',
				'link' => !empty($message['first_member_id']) ? '<a href="' . $boardurl . '/perfil/' . $message['memberName'] . '" title="' . $txt[92] . ' ' . $message['first_member_name'] . '">' . $message['first_member_name'] . '</a>' : $message['first_member_name']
			)
		),
		'board' => array(
			'id' => $message['ID_BOARD'],
			...
			'link' => '<a href="' . $boardurl . '/categoria/' . $message['description'] . '/">' . $message['bName'] . '</a>'
		)
	));
