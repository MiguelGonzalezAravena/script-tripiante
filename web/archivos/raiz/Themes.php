...
function ThemeAdmin()
{
	...
	if (!isset($_POST['submit']))
	{
		...
		$context['new_theme_dir'] = substr(realpath($boarddir . '/web/archivos/temas/default'), 0, -7);

		// Look for a non existent theme directory. (ie theme87.)
		$theme_dir = $boarddir . '/web/archivos/temas/theme';
		$i = 1;
...
function ThemeList()
{
	...
	$context['reset_dir'] = realpath($boarddir . '/web/archivos/temas');
	$context['reset_url'] = $boardurl . '/web/archivos/temas';
...
function PickTheme()
{
	global $txt, $db_prefix, $sc, $context, $modSettings, $user_info, $ID_MEMBER, $language, $boarddir;
...
function ThemeInstall()
{
	...
	if (!empty($_REQUEST['copy']) && $method == 'copy')
	{
		...
		$theme_dir = $boarddir . '/web/archivos/temas/' . preg_replace('~[^A-Za-z0-9_\- ]~', '', $_REQUEST['copy']);
	...
	elseif ($method = 'upload')
	{
		...
		$theme_dir = $boarddir . '/web/archivos/temas/' . $theme_name;
		...
		elseif (isset($_REQUEST['theme_gz']))
		{
			...
			$extracted = read_tgz_file($_REQUEST['theme_gz'], $boarddir . '/web/archivos/temas/' . $theme_name, false, true);
		}
		else
	...
	// Something go wrong?
	if ($theme_dir != '' && basename($theme_dir) != 'temas')
	{
		// Defaults.
		$install_info = array(
			'theme_url' => $boardurl . '/web/archivos/temas/' . basename($theme_dir),
			'images_url' => isset($images_url) ? $images_url : $boardurl . '/images',
			'theme_dir' => $theme_dir,
			'name' => $theme_name
		);
...
function CopyTemplate()
{
	...
	elseif (isset($_REQUEST['lang_file']) && preg_match('~^[^\./\\\\:\0]\.[^\./\\\\:\0]$~', $_REQUEST['lang_file']) != 0)
	{
		...
		$fp = fopen($boarddir . '/web/archivos/idiomas/' . $_REQUEST['lang_file'] . '.php', 'w');
		fwrite($fp, file_get_contents($filename));
		fclose($fp);
	...
	if (!empty($base_theme_dir))
	{
		...
		if (file_exists($boarddir . '/web/archivos/idiomas'))
		{
			$dir = dir($boarddir . '/web/archivos/idiomas');
			while ($entry = $dir->read())
			{
	...
	foreach ($lang_files as $file)
		$context['available_language_files'][$file] = array(
			...
			'value' => $file,
			'already_exists' => false,
			'can_copy' => file_exists($boarddir . '/web/archivos/idiomas') ? is_writable($boarddir . '/web/archivos/idiomas') : is_writable($theme_dir),
		);
	...
	if (file_exists($boarddir . '/web/archivos/idiomas'))
	{
		...
		while ($entry = $dir->read())
		{
			if (preg_match('~^([^\.]+\.[^\.]+)\.php$~', $entry, $matches) && isset($context['available_language_files'][$matches[1]]))
			{
				...
				$context['available_language_files'][$matches[1]]['can_copy'] = is_writable($boarddir . '/web/archivos/idiomas/' . $entry);
			}
		}
		$dir->close();
	}
