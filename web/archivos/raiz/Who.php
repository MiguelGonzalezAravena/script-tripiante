...
function Who() {
  global $db_prefix, $context, $scripturl, $txt, $modSettings, $memberContext;
...
function determineActions($urls) {
  ...
  foreach ($url_list as $k => $url) {
    ...
    // Some other normal action...?
      ...
      else if (($actions['action'] == 'post' || $actions['action'] == 'post2') && empty($actions['topic']) && isset($actions['board'])) {
        $data[$k] = $txt['who_hidden'];
        $board_ids[(int) $actions['board']][$k] = isset($actions['poll']) ? $txt['who_poll'] : $txt['who_post'];
      }
      // A subaction anyone can view... if the language string is there, show it.
      else if (isset($actions['sa']) && isset($txt['whoall_' . $actions['action'] . '_' . $actions['sa']]))
      ...
      else if (isset($txt['whoall_' . $actions['action']]))
        $data[$k] = $txt['whoall_' . $actions['action']];
      // Viewable if and only if they can see the board...
      else if (isset($txt['whotopic_' . $actions['action']])) {
      ...
      } else if (isset($txt['whopost_' . $actions['action']])) {
        ...
        list($ID_TOPIC, $subject) = mysqli_fetch_row($result);
