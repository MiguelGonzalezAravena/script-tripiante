...
{
	global $txt, $context, $scripturl, $settings, $modSettings, $ID_MEMBER;
...
</ul><div style="clear: both;"></div></div></div><div class="clearBoth"></div><div class="box_757"><div class="box_title" style="width: 757px;"><div class="box_txt box_757-34"><center>', $txt['bookmark_list'], '</center></div><div class="box_rss"><img alt="" src="' . $settings['images_url'] . '/blank.gif" style="width:16px;height:16px;" border="0"/></div></div><div style="width:747px;padding:4px;" class="windowbg"><form action="/favoritos/imagen/eliminar/" method="post">';

$RegistrosAMostrar		=	$modSettings['bookmarks_images'];
if(isset($_GET['pag'])) {
...
$NroRegistros	=	mysql_num_rows(db_query("SELECT ms.date, ms.points, ms.ID_PICTURE, ms.title, ms.ID_MEMBER, mem.realName, mem.ID_MEMBER, mem.memberName, bm.ID_MEMBER, bm.TYPE, bm.ID_TOPIC
FROM ({$db_prefix}bookmarks AS bm, {$db_prefix}gallery_pic AS ms, {$db_prefix}members AS mem)
WHERE bm.ID_MEMBER = $ID_MEMBER
AND ms.ID_PICTURE = bm.ID_TOPIC
AND mem.ID_MEMBER = ms.ID_MEMBER
