    ...
    if ($context['contar'] == 1 && $des['expire'] < time()) {
      ...
      echo '
        <a href="' . $boardurl . '/comunidades/' . $des['friendly_url'] . '/" title="' . $des['title'] . '" alt="' . $des['title'] . '">
          <img src="' . $des['logo'] . '" width="120px" height="120px" alt="' . $dest['title'] . '" class="avatar" title="' . $des['title'] . '" onerror="error_avatar(this)" />
        </a>
        <br />
        <div class="hrs"></div>
        <a href="' . $boardurl . '/comunidades/' . $des['friendly_url'] . '/" title="' . $des['title'] . '" alt="' . $des['title'] . '">
...
function template_crear() {
  ...
  foreach ($context['foro'] as $foro) {
    echo '
      <option value="' . $foro['ID_CATEGORY'] . '">' . $foro['name'] . '</option>';
  }
...
function template_editar2() {
  ...
  echo '
    ...
    <div class="comunidad_bloke1" style="width: 500px; margin-left: 200px;">
      <center>
        <b>
          <font color="green">¡Felicidades, tu comunidad se ha edito con &eacute;xito!</font>
        </b>
        <br /><br /><br />
        ...
        echo '
          <div class="comunidad_tema">
            <div>
              ...
              <div>
                <a style="color: #89601A; font-weight: bold; font-size: 13px;" href="' . $boardurl . '/comunidades/' . $row2['friendly_url'] . '/' . $row2['ID_TOPIC'] . '/' . ssi_amigable($row2['subject']) . '.html" target="_self" title="' . censorText($row2['subject']) . '" alt="' . censorText($row2['subject']) . '">' . ssi_reducir($row2['subject']) . '</a>
              </div>
            </div>
            <div class="size10">
              Por
              &nbsp;
              <a href="' . $boardurl . '/perfil/' . censorText($row2['posterName']) . '" target="_self" title="' . censorText($row2['posterName']) . '" alt="' . censorText($row2['posterName']) . '">' . censorText($row2['posterName']) . '</a>
            </div>
          </div>
          <div class="hrs"></div>';
      }
...
function template_ultimospost() {
  ...
  $NroRegistros	=	mysqli_num_rows(mysql_query("
  SELECT ct.ID_COMMUNITY, ct.ID_TOPIC, ct.locked, ct.isSticky, ct.ID_MEMBER, ct.subject, ct.posterName, ct.posterTime, ct.numViews, c.ID_COMMUNITY , c.friendly_url
  FROM ({$db_prefix}community_topic AS ct, {$db_prefix}communities AS c)
  WHERE ct.ID_COMMUNITY = c.ID_COMMUNITY
  AND c.friendly_url = '$id'
  ...
  echo '
    <p align="right" style="margin: 0px; padding: 0px;">
      <div align="right" class="paginacion2"><b>';
    ...
    <div align="center">
      <div class="box_errors">
        <div class="box_title" style="width: 388px">
          <div class="box_txt box_error" align="left">&iexcl;Atenci&oacute;n!</div>
          <div class="box_rss">
            <img alt="" src="' . $boardurl . '/blank.gif" style="width: 14px; height: 12px;" border="0" />
          </div>
        </div>
        <div class="windowbg" style="width: 380px; padding: 4px;">
          <br />
...
while($members	=	mysqli_fetch_assoc($request2)) {
...
$NroRegistros	=	mysqli_num_rows(db_query("SELECT cm.ID_MEMBER, cm.grade, cm.date, cm.name, cm.ID_COMMUNITY, c.ID_COMMUNITY, c.friendly_url, mem.ID_MEMBER, mem.gender, mem.avatar, mem.usertitle
FROM ({$db_prefix}community_members AS cm, {$db_prefix}communities AS c, {$db_prefix}members AS mem)
WHERE cm.ID_COMMUNITY = c.ID_COMMUNITY
AND c.friendly_url = '$id'
...
<div class="windowbg" style="width:531px;padding:4px;"><form style="margin: 0px; padding: 0px;" action="/?action=comunidades;sa=adminmiembro2;id=', $context['comunidad']['friendly_url'], '" method="POST" accept-charset="' . $context['character_set'] . '"><table>
...
<div class="windowbg" style="width:531px;padding:4px;"><form style="margin: 0px; padding: 0px;" action="/web/tp-comunidadesDen.php" method="POST" accept-charset="' . $context['character_set'] . '"><table>
...
echo '<div class="tagacom2"><ul style="margin-bottom:8px;"><li><a href="' . $boardurl . '/comunidades/" title="Comunidades">Comunidades</a></li> <li><a href="' . $boardurl . '/comunidades/categoria/', $context['comunidad']['friendly_url2'], '" title="', $context['comunidad']['bname'], '" alt="', $context['comunidad']['bname'], '">', $context['comunidad']['bname'], '</a></li> <li><a href="' . $boardurl . '/comunidades/', $context['comunidad']['friendly_url'], '" title="', $context['comunidad']['title'], '" alt="', $context['comunidad']['title'], '">', $context['comunidad']['title'], '</a></li> <li id="activer">Publicitar</li><div style="clear: both;"></div></ul></div>';
panel();	
