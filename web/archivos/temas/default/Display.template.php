...
function template_main() {
  ...
  while ($message = $context['get_message']()) {
    ...
    if (!empty($settings['show_user_images']) && empty($options['show_no_avatars']) && !empty($message['member']['avatar']['name'])) {
      echo '<div class="fondoavatar" style="overflow: hidden; width: 130px;" align="center"><a href="' . $boardurl . '/perfil/' . $message['member']['username'] . '" title="Ver perfil"><img src="' . $message['member']['avatar']['name'] . '" width="105" alt="" class="avatar" border="0" onerror="error_avatar(this)" /></a><br /><span class="mp">' . $message['member']['blurb'] . '</span></div></center><br/>';
    } else {
      echo '<div class="fondoavatar" style="overflow: auto; width: 130px;" align="center"><a href="' . $boardurl . '/perfil/' . $message['member']['username'] . '" title="Ver perfil"><img src="' . $boardurl . '/avatar.gif" alt="Sin avatar" border="0"></a><br /><span class="mp">' . $message['member']['blurb'] . '</span></div></center><br />';
    }

    echo '<b><a href="' . $boardurl . '/perfil/' . $message['member']['username'] . '" style="font-size: 14px; color: #FF6600;">' . $message['member']['name'] . '</a></b><br />';
    ...
    if ($settings['show_profile_buttons']) {
      if ($context['can_send_pm']) {
        echo '<div style="margin-bottom: 2px;"><span style="font-size: 12px;"><img alt="" src="' . $settings['images_url'] . '/icons/mensaje_para.gif" border="0" /> <a href="' . $boardurl . '/mensajes/a/' . $message['member']['username'] . '" title="Enviar mensaje">Enviar mensaje</a></span></div>';
      }

      if ($context['user']['is_logged'])	 {
        echo '<div style="margin-bottom: 4px;"><span class="icons fot2" style="font-size: 12px;"><a href="' . $boardurl . '/imagenes/' . $message['member']['name'] . '" title="Sus im&aacute;genes"> Sus im&aacute;genes</a></span></div>';
      }
    }
    ...
    if ($context['user']['is_guest']) {
      echo '<div align="center" style="-moz-border-radius: 5px; -webkit-border-radius: 5px; display: block; margin-bottom: 25px; margin-top: 10px; padding: 2px; border: solid 1px #D5CCC3; background: #FFF;">' . $modSettings['horizontal'] . '<br />
    ...
    if ($context['user']['is_guest']) {
      echo '<div align="center" style="-moz-border-radius: 5px; -webkit-border-radius: 5px; display: block; margin-bottom: 10px; margin-top: 25px; padding: 2px; border: solid 1px #D5CCC3; background: #FFF;">' . $modSettings['horizontal'] . '<br />
          <a href="' . $boardurl . '/registrarse/" style="font-size: 12px; color: #FFB600; margin-bottom: 3px;">
            <b>REG&Iacute;STRATE GRATIS Y ELIMINA ESTA PUBLICIDAD, ADEM&Aacute;S TENDR&Aacute;S ACCESO A TODOS LOS POSTS Y FUNCIONES</b>
          </a>
          <br />
    ...
    if($context['allow_admin'] || $iduser == $context['user']['id']) {
      echo '<input class="login" style="font-size: 11px;" value="Editar post" title="Editar post" onclick="location.href=\'' . $boardurl . '/editar-post/id-', $context['current_topic'], '\'" type="button"> <input class="login" style="font-size: 11px;" value="Eliminar post" title="Eliminar post" onclick="if (!confirm(\'\xbfEstas seguro que desea eliminar este post?\')) return false; return errorrojo2(this.form.causa.value);" type="submit">
      ...
      if ($message['can_remove']) {
        echo '<input class="login" style="font-size: 11px;" value="Editar post" title="Editar post" onclick="location.href=\'' . $boardurl . '/editar-post/id-', $context['current_topic'], '\'" type="button"> <input class="login" style="font-size: 11px;" value="Eliminar post" title="Eliminar post" onclick="if (!confirm(\'\xbfEstas seguro que desea eliminar este post?\')) return false; location.href=\'' . $boardurl . '/eliminar-post/', $context['current_topic'], '/\'" type="button">
        <div class="hrs"></div>';
      }
    }
    ...
    if ($context['Conocido'] || $context['Vecino'] || $context['Amigo'] || $context['Familiar'] || $context['Casero'] || $context['allow_admin']) {
      echo '<span id="span_opciones1" class="size10">
    ...
    if($context['user']['is_logged']) {
      echo '<center>
        <span id="span_opciones2" style="text-align: center; display: block;">
          ...
          <a class="icons recomendar_post" href="' . $boardurl . '/enviar-a-amigo/' . $context['current_topic'] . '">Enviar a un amigo</a></span></center>';
    } else {
    ...
    echo '<div class="hrs"></div>
            <div style="float: left; margin-right: 4px;"><b>Agregar a:</b></div>
    ...
    if (!empty($message['member']['signature']) && empty($options['show_no_signatures'])) {
      echo '<div class="box_390" style="margin-top: 8px;">
          <div class="box_title" style="width: 384px;">
    ...
    echo '<div class="box_title" style="width: 772px;">
    ...
    if ($context['haycom']) {
      foreach ($context['comentarios'] as $coment) {
        ...
        if ($context['user']['is_logged']) {
          echo ' <a  href="' . $boardurl . '/mensajes/a/' . $coment['nommem'] . '" title="Enviar MP a: ' . $coment['nomuser'] . '">
