...
function template_main() {
    ...
    <div class="tborder">
      <form action="' . $scripturl . '?action=membergroups;sa=add;generalgroup" method="post" accept-charset="', $context['character_set'], '" style="margin: 0;">
        <table width="100%" cellpadding="2" cellspacing="1" border="0">
          ...
          <tr class="catbg3">
            <td width="12%">', $txt['membergroups_name'], '</td>
            <td width="12%" align="center">', $txt['membergroups_stars'], '</td>
            <td width="12%" align="center">', $txt['membergroups_members_top'], '</td>
            <td width="12%" align="center">', $txt[17], '</td>
          </tr>';
  foreach ($context['groups']['regular'] as $group) {
    echo '
          <tr>
            <td class="windowbg">', empty($group['color']) ? ( $group['can_search'] ? $group['link'] : $group['name'] ) : '<span style="color: ' . $group['color'] . '">' . ( $group['can_search'] ? $group['link'] : $group['name']) . '</span>', $group['id'] == 1 ? ' (<a href="' . $scripturl . '?action=helpadmin;help=membergroup_administrator" onclick="return reqWin(this.href);">?</a>)' : ($group['id'] == 3 ? ' (<a href="' . $scripturl . '?action=helpadmin;help=membergroup_moderator" onclick="return reqWin(this.href);">?</a>)' : ''), '</td>
            <td class="windowbg" align="left">', $group['stars'], '</td>
            <td class="windowbg" align="center">', $group['num_members'], '</td>
            <td class="windowbg" align="center"><a href="' . $scripturl . '?action=membergroups;sa=edit;group=' . $group['id'] . '">' . $txt['membergroups_modify'] . '</a></td>
          </tr>';
  }
    ...
    <div class="tborder">
      <form action="' . $scripturl . '?action=membergroups;sa=add" method="post" accept-charset="', $context['character_set'], '" style="margin: 0;">
        <table width="100%" border="0" cellpadding="2" cellspacing="1">
          ...
          <tr class="windowbg">
            <td width="1%">', $txt['membergroups_name'], '</td>
            <td width="1%" align="center">', $txt['membergroups_stars'], '</td>
            <td width="1%" align="center">', $txt['membergroups_min_posts'], '</td>
  ...
  foreach ($context['groups']['post'] as $group)
  {
    echo '
          <tr>
            <td class="windowbg">', empty($group['color']) ? ($group['can_search'] ? $group['link'] : $group['name']) : '<span style="color: ' . $group['color'] . '">' . ($group['can_search'] ? $group['link'] : $group['name']) . '</span>', '</td>
            <td class="windowbg" align="left">', $group['stars'], '</td>
