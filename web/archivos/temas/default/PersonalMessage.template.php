...
function template_folder() {
  ...
  if (!isset($context['sl-singlepm'])) {
      ...
      <table class="linksList" style="width: 754px;">
        <thead align="center">
          <tr>
            <th>&nbsp;</th>
            <th>', $txt[319], '</th>
            <th>', ($context['from_or_to'] == 'from' ? $txt[318] : $txt[324]), '</th>
            <th>', ($context['from_or_to'] == 'from' ? $txt[317] : $txt['317b']), '</th>
          </tr>
        </thead>
        <tbody>';
    ...
    while ($message = $context['get_pmessage']()) {
      echo '<tr>
      <td>
      <a title="Eliminar mensaje" href="' . $boardurl . '/mensajes/' . (empty($_REQUEST['f']) || $_REQUEST['f'] == 'inbox' ? 'recibidos' : ($_REQUEST['f'] == 'outbox' ? 'enviados' : '')) . '/eliminar-' . $message['id'] . '"  onclick="if (!confirm(\'\xbfEstas seguro que desea eliminar este mensaje?\')) return false;">
    ...
    echo '
      ...
      echo '
        <div class="noesta" style="width: 754px;">
          ' . $txt[151] .
          (empty($_REQUEST['f']) || $_REQUEST['f'] == 'inbox' ? 'recibidos' : ($_REQUEST['f'] == 'outbox' ? 'enviados' : '')) . '
          ...
        </div>';
    }
  }
  ...
  if ((isset($context['sl-singlepm']) || (isset($modSettings['enableSinglePM']) && $modSettings['enableSinglePM'] == 0) || !isset($modSettings['enableSinglePM']))) {
    while ($message = $context['get_pmessage']()) {
      ...
      echo '
        <div style="margin-left: 8px; width: 757px; float: left;">
          <a name="mp_', $message['id'], '"></a>
          <div class="box_757">
            ...
            <table class="windowbg" cellpadding="3" cellspacing="0" border="0" style="text-align: left; width: 754px;">
              <tr>
                ...
                <td style="width: 691px; text-align: left;" valign="top">
                  <a href="' . $boardurl . '/perfil/', $message['member']['username'], '">', $message['member']['name'], '</a>
                </td>
              </tr>
              <tr>
                <td align="left" style="width: 50px; text-align: left;" valign="top">
            ...
            <p align="center">
              <input class="login" style="font-size: 11px;" value="Eliminar MP" title="Eliminar MP" onclick="if (!confirm(\'\xbfEstas seguro que desea eliminar este mensaje?\')) return false; location.href=\'' . $boardurl . '/mensajes/recibidos/eliminar-', $message['id'], '\'" type="button" />
              &nbsp;
              <input class="login" style="font-size: 11px;" value="Responder MP" title="Responder MP" onclick="location.href=\'' . $boardurl . '/mensajes/responder/a/', $message['member']['name'], '/id/', $message['id'], '\'" type="button" />
            </p>';
    }
  }
...
function template_send() {
  ...
  echo '
    <div style="margin-left: 8px; width: 757px; float: left;">
      <script language="JavaScript" type="text/javascript">
        function errorrojo3 (to, subject, cuerpo_comment) {
          if (to == \'' . $context['user']['name'] . '\') {
            document.getElementById(\'errors3\').innerHTML = \'<br /><font class="size10" style="color: red;">No te puedes autoenviar mensaje.</font>\';
            return false;
          }

          if (to == \'"' . $context['user']['name'] . '"\') {
            document.getElementById(\'errors7\').innerHTML = \'<br /><font class="size10" style="color: red;">No te puedes autoenviar mensaje.</font>\';
            return false;
          }

          if (to == \'\') {
            document.getElementById(\'errors4\').innerHTML = \'<br /><font class="size10" style="color: red;">Debes escribir el destinario del mensaje.</font>\';
            return false;
          }

          if (titulo == \'\') {
            document.getElementById(\'errors5\').innerHTML = \'<br /><font class="size10" style="color: red;">Debes escribir un asunto.</font>\';
            return false;
          }

          if (cuerpo_comment == \'\') {
            document.getElementById(\'errors6\').innerHTML = \'<br /><font class="size10" style="color: red;">Debes escribir un mensaje.</font>\';
            return false;
          }
        }
      </script>
      <div class="box_757">
        <div class="box_title" style="width: 752px;">
          <div class="box_txt box_757-34">
            <center>' . $txt[321] . '</center>
          </div>
          <div class="box_rss">
            <img src="' . $settings['images_url'] . '/blank.gif" style="width: 16px; height: 16px;" border="0">
          </div>
        </div>
      </div>
      <form action="' . $boardurl . '/mensajes/enviar/" method="post" accept-charset="' . $context['character_set'] . '" name="postmodify" id="postmodify">
          ...
          <table cellpadding="3" cellspacing="1">
            <tr>
              <td>
                ...
                <label id="errors3"></label>
                <label id="errors7"></label>
                <label id="errors4"></label>
                <br />
                <font class="size11">
                  <b>' . $txt[70] . ':</b>
                </font>
  ...
  if ($context['visual_verification']) {
    ...
    if ($context['use_graphic_library']) {
      echo '
        <img src="' . $context['verificiation_image_href'] . '" alt="' . $txt['pm_visual_verification_desc'] . '" />
        <br />';
    } else {
      echo '
        ...
        <img src="' . $context['verificiation_image_href'] . ';letter=5" alt="' . $txt['pm_visual_verification_desc'] . '" />
        <br />';
    }

    echo '
          <a href="' . $context['verificiation_image_href'] . ';sound" onclick="return reqWin(this.href, 400, 120);">' . $txt['pm_visual_verification_listen'] . '</a>
          <br /><br />
  ...
  echo '
                <center>
                  <input class="login" type="submit" value="', $txt[148], '" tabindex="', $context['tabindex']++, '" onclick="return errorrojo3(this.form.to.value, this.form.subject.value, this.form.cuerpo_comment.value);"/>
                </center>
    ...
    <table border="0" width="80%" cellpadding="4" cellspacing="1" class="bordercolor" align="center">
      ...
      <tr>
        <td class="windowbg">
          ...
          <br />
          <b>
            <a href="' . $scripturl . '?action=pm;sa=removeall2;f=' . $context['folder'] . ';' . ($context['current_label_id'] != -1 ? ';l=' . $context['current_label_id'] : '') . ';sesc=' . $context['session_id'] . '">' . $txt[163] . '</a>
            &nbsp;-&nbsp;
            <a href="javascript:history.go(-1);">' . $txt[164] . '</a>
          </b>
        </td>
      </tr>
    </table>';
        ...
        <table width="100%" class="windowbg">
          <tr class="windowbg2">
            <td colspan="2" style="padding: 1ex;">
              <center>
                <span class="smalltext">' . $txt['pm_labels_desc'] . '</span>
              </center>
            </td>
          </tr>
          <tr>
    ...
    echo '
          <tr>
            <td align="center" colspan="2">
              <br />
              <input class="login" type="submit" name="save" value="Guardar Cambio" style="font-weight: normal;" />
...
function template_quickreply_box() {
  global $context, $db_prefix, $settings, $txt, $boardurl;

  $idPrivateMessage = (int) $_REQUEST['pmsg'];

  if (!empty($idPrivateMessage)) {
    $request = db_query("
      SELECT *
      FROM ({$db_prefix}personal_messages) 
      WHERE ID_PM = ".$idPrivateMessage, __FILE__, __LINE__);

    while ($row = mysqli_fetch_assoc($request)) {
      censorText($row['body']);
      $row['body'] = trim(un_htmlspecialchars(strip_tags(strtr(parse_bbc($row['body'], false, $idPrivateMessage), array('<br />' => "\n", '</div>' => "\n", '</li>' => "\n", '&#91;' => '[', '&#93;' => ']')))));
      $comentario = $row['body'];
      $fecha = $row['msgtime'];
      $nombre = $row['fromName'];
    }
  ...
  echo '
    <b class="size11">Mensaje Privado:</b>
    <br />';
