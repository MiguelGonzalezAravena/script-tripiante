        ...
        <table border="0" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF" width="100%" >
          <tr>
            <td width="50%" colspan="2"  align="center" class="catbg">
              <b>' . $txt['pcomments_addcomment'] . '</b>
            </td>
          </tr>
          <tr>
            <td width="28%" class="windowbg2" align="right">
              <span class="gen">
                <b>' . $txt['pcomments_subject'] . '</b>
              </span>
            </td>
...
// Check if comments are autoapproved
  ...
  echo '
            <input type="submit" value="' . $txt['pcomments_addcomment'] . '" name="submit" />
          </td>
        </tr>
      </table>
    </form>';
...
function template_edit() {
  ...
  $dbresult = db_query("
    SELECT ID_COMMENT, ID_MEMBER, comment, subject, date, COMMENT_MEMBER_ID 
    ...
    <form method="POST" name="cprofile" id="cprofile" action="' . $scripturl . '?action=comment&sa=edit2">
      <table border="0" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF" width="100%">
        ...
        <tr>
          ...
          <td width="72%" class="windowbg2">
            <input type="text" name="subject" size="64" maxlength="100" value="' . $row['subject'] . '" />
          </td>
        </tr>
        <tr>
          <td width="28%" valign="top" class="windowbg2" align="right">
  ...
  echo '
            <input type="submit" value="' . $txt['pcomments_editcomment'] . '" name="submit" />
          </td>
        </tr>
      </table>
    </form>';
...
function template_commentsadmin() {
  ...
  $request = db_query("
    SELECT p.ID_COMMENT, p.ID_MEMBER, p.comment, p.subject, p.date, m.realName, p.COMMENT_MEMBER_ID, m2.realName ProfileName 
  ...
  while ($row = mysqli_fetch_assoc($request)) {
    echo '
      <tr>
        ...
        <td>
          <a href="' . $scripturl . '?action=profile;u='  . $row['COMMENT_MEMBER_ID'] . '">' . $row['ProfileName'] . '</a>
        </td>
        <td>' . timeformat($row['date']) . '</td>
        <td>
          <a href="' . $scripturl . '?action=comment;sa=approve;id=' . $row['ID_COMMENT'] . '">' . $txt['pcomments_approve'] . '</a>
          <br />
          <a href="', $scripturl, '?action=comment;sa=delete;id=' . $row['ID_COMMENT'] . '">',$txt['pcomments_delcomment'],'</a>
        </td>
      </tr>';
  }
  ...
  echo '
          ...
          <br />
          <b>Has Profile Comments helped you?</b>
          &nbsp;
          Then support the developers:
          <br />
...
function ProfileCommentsCopyright() {
  ...
  echo '
    <div align="center">
      <!--Link must remain or contact me to pay to remove.-->
      Powered by <a href="http://www.smfhacks.com" target="blank">Profile Comments</a>
      <!--End Copyright link-->
    </div>';
}
