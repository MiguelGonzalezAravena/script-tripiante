    ...
    <table border="0" width="400" cellspacing="0" cellpadding="3" align="center" class="tborder">
      ...
      <tr class="windowbg">
        <td align="center" style="padding-top: 2ex; padding-bottom: 1ex;">
          <b>
            <label for="subname">' . $txt['smf254'] . '</label>:
          </b>
          &nbsp;
          <input type="text" name="subname" id="subname" value="' . $context['message']['subject'] . '" size="25" />
          <br /><br />
          <input type="radio" name="step2" value="onlythis" checked="checked" class="check" />
          &nbsp;
          ' . $txt['smf255'] . '
          <br />
          <input type="radio" name="step2" value="afterthis" class="check" />
          &nbsp;
          ' . $txt['smf256'] . '
          <br />
          <input type="radio" name="step2" value="selective" class="check" />
          &nbsp;
          ' . $txt['smf257'] . '
          <br />
          <br />
    ...
    <table border="0" width="400" cellspacing="1" class="bordercolor" cellpadding="4" align="center">
      ...
      <tr>
        <td class="windowbg" valign="middle" align="center">
          ' . $txt['smf259'] . '
          <br /><br />
          <a href="' . $scripturl . '?board=' . $context['current_board'] . '.0">' . $txt[101] . '</a>
          <br />
          <a href="' . $scripturl . '?topic=' . $context['old_topic'] . '.0">' . $txt['smf260'] . '</a>
          <br />
    ...
    echo '
        <tr class="windowbg" id="not_selected_' . $message['id'] . '">
          ...
          <td valign="middle" align="center" width="5%">
            <a href="' . $scripturl . '?action=splittopics;sa=selectTopics;subname=' . $context['topic']['subject'] . ';topic=' . $context['topic']['id'] . '.' . $context['not_selected']['start'] . ';start2=' . $context['selected']['start'] . ';move=down;msg=' . $message['id'] . '" onclick="return select(\'down\', ' . $message['id'] . ');">
              <img src="' . $settings['images_url'] . '/split_select.gif" alt="-&gt;" />
            </a>
          </td>
        </tr>';
  }
      ...
      <table id="table_selected" border="0" width="98%" cellspacing="1" class="bordercolor" cellpadding="4" align="center">
        <tr class="titlebg">
          <td colspan="2">
            ...
            &nbsp;
            (<a href="' . $scripturl . '?action=splittopics;sa=selectTopics;subname=' . $context['topic']['subject'] . ';topic=' . $context['topic']['id'] . '.' . $context['not_selected']['start'] . ';start2=' . $context['selected']['start'] . ';move=reset;msg=0" onclick="return select(\'reset\', 0);">' . $txt['split_reset_selection'] . '</a>)
          </td>
        </tr>
        <tr class="windowbg">
          <td colspan="2" valign="middle">
      ...
      echo '
        <tr class="windowbg" id="selected_' . $message['id'] . '">
          <td width="5%" valign="middle" align="center">
            <a href="' . $scripturl . '?action=splittopics;sa=selectTopics;subname=' . $context['topic']['subject'] . ';topic=' . $context['topic']['id'] . '.' . $context['not_selected']['start'] . ';start2=' . $context['selected']['start'] . ';move=up;msg=' . $message['id'] . '" onclick="return select(\'up\', ' . $message['id'] . ');">
              <img src="' . $settings['images_url'] . '/split_deselect.gif" alt="&lt;-" />
            </a>
          </td>
          <td class="smalltext">
            ' . $message['subject'] . ' - ' . $message['poster'] . '
...
function template_merge() {
  ...
  if (!empty($context['boards']) && count($context['boards']) > 1) {
    ...
    echo '
        ...
        <noscript>
          <input type="submit" value="' . $txt[462] . '" />
        </noscript>
      </td>';
  }
...
function template_merge_extra_options() {
  ...
  echo $txt['merge_select_subject'] . ': <select name="subject" onchange="this.form.customSubject.disabled = this.options[this.selectedIndex].value != 0;">';
  ...
  echo '
        ...
        <input type="text" name="custom_subject" size="60" disabled="disabled" id="customSubject" />
        <br />
        <br />
        <input type="checkbox" class="check" name="enforce_subject" value="1" />
  ...
  if (!empty($context['boards']) && count($context['boards']) > 1) {
    ...
    foreach ($context['boards'] as $board) {
      echo '
        <tr>
          <td>
            <input type="radio" name="board" value="' . $board['id'] . '"' . ($board['selected'] ? ' checked="checked"' : '') . ' class="check" />
            &nbsp;
            ' . $board['name'] . '
          </td>
        </tr>';
  ...
  if (!empty($context['polls'])) {
    ...
    foreach ($context['polls'] as $poll) {
      echo '
        <tr>
          <td>
            <input type="radio" name="poll" value="' . $poll['id'] . '"' . ($poll['selected'] ? ' checked="checked"' : '') . ' class="check" />
            &nbsp;
            ' . $poll['question'] . '
            &nbsp;
            (
            ' . $txt[118] . ':
            &nbsp;
            <a href="' . $scripturl . '?topic=' . $poll['topic']['id'] . '.0" target="_blank">' . $poll['topic']['subject'] . '</a>
            )
          </td>
        </tr>';
    }
    ...
    echo '
            <tr>
              <td>
                <input type="radio" name="poll" value="-1" class="check" />
                &nbsp;
                (' . $txt['merge_no_poll'] . ')
              </td>
            </tr>
