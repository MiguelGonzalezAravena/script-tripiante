...
function template_main() {
  ...
  foreach($brokendown AS $group => $members) {
    echo '
      <table class="linksList">
        <thead align="center">
          <tr>
            <th>
              <a href="' . $scripturl . '?action=who;start=', $context['start'], ';sort=user', $context['sort_direction'] != 'down' && $context['sort_by'] == 'user' ? '' : ';asc', '">';

    if ($group == 'Miembros') {
      echo 'Miembros';
    } else if ($group == 'Visitantes') {
      echo 'Visitantes';
    } else if ($group == 'Spiders') {
      echo 'Spiders';
    }
    ...
    echo '
          <th>
            <a href="' . $scripturl . '?action=who;start=' . $context['start'] . ';sort=time' . ($context['sort_direction'] == 'down' && $context['sort_by'] == 'time' ? ';asc' : '') . '">' . $txt['who_time'] . ' ' . ($context['sort_by'] == 'time' ? '' : '') . '</a>
          </th>
          <th>' . $txt['who_action'] . '</th>
        </tr>
      </thead>
      <tbody>';
    ...
    foreach ($members as $member) {
      ...
      echo '
          <span' . ($member['is_hidden'] ? ' style="font-style: italic;"' : '') . '>
            ' . ($member['is_guest'] ? $member['name'] : '<a href="' . $member['href'] . '" title="' . $txt[92] . ' ' . $member['name'] . '"' . (empty($member['color']) ? '' : ' style="color: ' . $member['color'] . '"') . '>' . $member['name'] . '</a>') . '
          </span>
        </td>';
  ...
  echo '
    <br />
    <div class="noesta">Tenemos un total de ' . $contar++ . ' usuarios conectados...</div>
    <br />';

  echo '
    <tr class="linksList">
      <td align="center"><b>', $txt[139], ':</b> ' . $context['page_index'] . '</td>
    </tr>';
}
