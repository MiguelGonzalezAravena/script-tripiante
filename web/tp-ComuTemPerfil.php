...
if($rows == 0) {
  ...
  while ($row=mysqli_fetch_assoc($request)) {
    ...
    <td style="text-align: left;">
      <a href="/comunidades/' . $row['friendly_url'] . '/' . $row['ID_TOPIC'] . '/' . ssi_amigable($row['subject']) . '.html"  class="titlePost" alt="' . $row['subject'] . '" title="' . $row['subject'] . '">' . reducir30($row['subject']) . '</a>
    </td>
    <td>
      <a title="' . $row['title'] . '" alt="' . $row['title'] . '" href="/comunidades/' . $row['friendly_url'] . '">' . reducir22($row['title']) . '</a>
    </td>';
  ...
  if ($PagAct > 1)
    echo '<a style="cursor: pointer;" onclick="ComuTemPerfil(\''  . $user . '\',\'' . $PagAnt . '\');">&#171; anterior</a>';
  if ($PagAct < $PagUlt)
    echo '<a style="cursor: pointer;" onclick="ComuTemPerfil(\''  . $user . '\',\'' . $PagSig . '\');">siguiente &#187;</a>';
